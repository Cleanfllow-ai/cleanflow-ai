...
import { ProcessingWizardProvider, useProcessingWizard, STEP_ORDER_EXISTING, STEP_ORDER_NEW } from "./WizardContext"
...
// Reads step from context to render dynamic step subtitle in the dialog header
function WizardStepSubtitle() {
    ...
    if (mode === "new") {
        return <p className="text-sm text-muted-foreground mt-0.5">Import a file to your Data Catalog</p>
    }

    return (
        <p className="text-sm text-muted-foreground mt-0.5">
            Step {currentIndex + 1} of {total} — {STEP_LABELS[step] ?? step}
        </p>
    )
}
