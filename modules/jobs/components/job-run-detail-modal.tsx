...
function getStatusColor(status: string) {
    switch (status) {
        ...
        case "NO_EXPORTABLE_ROWS": return "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-500/20 dark:text-slate-300 dark:border-slate-500/30"
        case "SKIPPED": return "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-500/20 dark:text-slate-300 dark:border-slate-500/30"
...
function getFileStatusColor(status: string | undefined) {
    ...
    if (s.includes("FAILED")) return "bg-red-100 text-red-700 border-red-200 dark:bg-red-500/15 dark:text-red-500 dark:border-red-500/25"
    ...
    if (s.includes("QUARANTINED")) return "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-500/15 dark:text-amber-500 dark:border-amber-500/25"
            ...
            <DialogContent className="sm:max-w-[750px] max-h-[85vh] overflow-y-auto">
                    ...
                    <div className="rounded-lg border border-amber-500/30 bg-amber-100/60 dark:bg-amber-500/5 p-3 space-y-2">
                        ...
                        {partialQuarantineUploadId && (
                            <Button
                                variant="outline"
                                size="sm"
                                className="h-7 text-xs gap-1.5 border-amber-500/40 text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/10 mt-1"
                                        ...
                                        {(result.quarantined ?? 0) > 0 && uploadId && (
                                            <div className="flex items-center gap-2 ml-5">
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    className="h-7 text-xs gap-1.5 border-amber-500/30 text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/10"
                ...
                {entityEntries.some(([, r]) => r.error) && (
                    <div className="space-y-2">
                        ...
                        <div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-500/20 dark:bg-red-500/10 space-y-2 text-xs">
                            {entityEntries.map(([entity, result]) => {
                ...
                {run.pipeline_logs?.some(l => l.phase === "export" && l.details?.errors?.length) && (
                    <div className="space-y-2">
                        ...
                        <div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-500/20 dark:bg-red-500/10 space-y-1 text-xs max-h-[200px] overflow-y-auto">
                            {run.pipeline_logs
