            ...
            >
                ...
                <div className="space-y-5 py-1">
                                    ...
                                    <div className="border rounded-md max-h-40 overflow-y-auto p-2 space-y-1">
                                        {d.availableEntities.map(entity => (
                                            <label
                                                key={entity.value}
                                                className={cn(
                                                    "flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer text-sm hover:bg-accent/50 transition-colors",
                                                    d.entities.includes(entity.value) && "bg-accent"
                                                )}
                                            >
                                                <Checkbox
                                                    checked={d.entities.includes(entity.value)}
                                                    onCheckedChange={() => d.toggleEntity(entity.value)}
                    ...
                    <div className="flex items-center justify-between py-2 border-t border-border/50">
                        <div>
                            <Label className="text-sm font-medium">Data Quality Check</Label>
                            <p className="text-xs text-muted-foreground mt-0.5">
                                Run default DQ validation on transferred data
                            </p>
                        </div>
                        <Switch
                            checked={d.dqEnabled}
                            onCheckedChange={d.setDqEnabled}
                        />
                    </div>
                </div>
